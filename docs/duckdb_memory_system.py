...
class ChatMemoryService:
    ...
    사용 예시:
        ...
        # 세션 생성
        session = memory.create_session("user123", "RNA 연구 대화")
        
        # 메시지 추가
        memory.add_message(session['session_id'], "user", "RNA 시퀀싱이 뭐야?")
        memory.add_message(session['session_id'], "assistant", "RNA 시퀀싱은...")
        
        # 메시지 조회
        messages = memory.get_messages(session['session_id'])
        
        # 대화 검색
        results = memory.search_in_history(session['session_id'], "RNA")
    """
...
class SearchAnalyticsService:
    ...
    사용 예시:
        ...
        # 인기 검색어 조회
        popular = analytics.get_popular_queries(days=7, limit=10)
    ...
    def track_search(self, query: str, user_id: str = None, query_type: str = "semantic"):
        ...
        사용 예시:
            with analytics.track_search("RNA", "user123") as tracker:
                results = do_search("RNA")
                tracker.set_results(len(results), results[0]['score'] if results else None)
        """
        class SearchTracker:
...
def example_usage():
    ...
    print("=" * 60)
    print("DuckDB Memory System - 사용 예제")
    print("=" * 60)
    
    # 인메모리 모드로 테스트
    db = get_duckdb(':memory:')
    
    # 1. 대화 메모리 테스트
    print("\n[1] 대화 메모리 테스트")
    print("-" * 40)
    
    memory = ChatMemoryService(':memory:')
    ...
    # 메시지 추가
    memory.add_message(session['session_id'], "user", "RNA 시퀀싱이 뭐야?")
    memory.add_message(session['session_id'], "assistant", "RNA 시퀀싱은 RNA 분자의 염기서열을 결정하는 기술입니다.", tokens_used=50)
    memory.add_message(session['session_id'], "user", "어떤 종류가 있어?")
    memory.add_message(session['session_id'], "assistant", "주요 RNA 시퀀싱 방법으로는 RNA-Seq, small RNA-Seq, single-cell RNA-Seq 등이 있습니다.", tokens_used=80)
    ...
    # 컨텍스트 조회
    context = memory.get_recent_context(session['session_id'], max_messages=4)
    print(f"\nLLM 컨텍스트: {len(context)} 메시지")
    
    # 검색
    results = memory.search_in_history(session['session_id'], "RNA")
    print(f"\n'RNA' 검색 결과: {len(results)}개")
    ...
    analytics = SearchAnalyticsService(':memory:')
    
    # 검색 로그 저장
    test_queries = [
        ("RNA sequencing", 15, 0.92),
        ("CRISPR", 8, 0.88),
        ("gene expression", 12, 0.85),
        ("RNA sequencing", 10, 0.90),
        ("protein folding", 5, 0.75),
        ("RNA sequencing", 20, 0.95),
    ]
    
    for query, count, score in test_queries:
        analytics.log_search(
            query=query,
            user_id="user123",
            query_type="semantic",
    ...
    # 인기 검색어
    popular = analytics.get_popular_queries(days=7, limit=5)
    print("인기 검색어:")
    for p in popular:
        print(f"  - {p['query']}: {p['search_count']}회")
    
    # 성능 통계
    perf = analytics.get_performance_stats(days=7)
    print(f"\n성능 통계:")
    print(f"  - 총 검색: {perf['total_searches']}회")
    print(f"  - 평균 응답시간: {perf['avg_response_time_ms']}ms")
    print(f"  - 평균 점수: {perf['avg_top_score']}")
    
    # 3. 검색 시간 자동 측정 테스트
    print("\n[3] 검색 시간 자동 측정 테스트")
    print("-" * 40)
    
    with analytics.track_search("test query", "user123", "semantic") as tracker:
        # 실제로는 여기서 검색 수행
        import time
        time.sleep(0.1)  # 검색 시뮬레이션
        tracker.set_results(10, 0.95)
    ...
    docs = DocumentMetadataService(':memory:')
    ...
    docs.upsert_document(
        ...
        abstract="This review covers recent advances in RNA sequencing...",
    ...
    docs.upsert_document(
        ...
        abstract="CRISPR technology has revolutionized...",
    ...
    # 문서 검색
    results = docs.search_documents(keyword="RNA")
    print(f"'RNA' 검색 결과: {len(results)}개")
    
    # 대기 문서
    pending = docs.get_pending_documents()
    print(f"임베딩 대기 문서: {len(pending)}개")
    
    # 통계
    doc_stats = docs.get_stats()
    print(f"\n문서 통계: {doc_stats}")
